...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        //获取事先转入的流动性数值
        //正常情况下，交易对合约本身没有任何流动性代币，所有的流动性代币都在流动性提供者手中
        uint liquidity = balanceOf[address(this)];
        
        //计算手续费 提取资产不涉及流动性增发，但这里还是要计算并发送手续费，否则用户
        //提取资产时就会计算不准确（？）
        bool feeOn = _mintFee(_reserve0, _reserve1);
        uint _totalSupply = totalSupply; // gas savings, must be defined here since totalSupply can update in _mintFee
        ...
        //将用户转入本合约的流动性销毁，因为此时流动性代币已经转移到交易对
    ...
    // this low-level function should be called from a contract which performs important safety checks
    //两种ERC20代币相互买卖
    //bytes calldata data 接收后执行回调时的传递数据
    //lock 修饰符 防止重入
    //amountOut 购买的token数量 
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        //如果参数data不为空，那么执行调用合约的uniswapCall回调函数并将data传递过去
        //普通调用时这个data为空
        ...
        //恒定乘积验证
        //xin 和 yin 是注入的资产数量，所以要扣除千分之三的手续费
    ...
    //强制交易对合约中两种代币实际余额和保存的恒定乘积中的资产数量一致（多余的发送给调用者）
